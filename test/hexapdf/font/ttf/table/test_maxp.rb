...
require 'test_helper'
require 'stringio'
require 'hexapdf/font/ttf/table/maxp'

describe HexaPDF::Font::TTF::Table::Maxp do
  before do
    data = [1, 0, 10, 11, 12, 13, 14, 2, 15, 16, 17, 18, 19, 20, 21, 22].pack('n*')
    io = StringIO.new(data)
    @file = Object.new
    @file.define_singleton_method(:io) { io }
    @entry = HexaPDF::Font::TTF::Table::Directory::Entry.new('hhea', 0, 0, io.length)
  end

  describe "initialize" do
    it "reads the version 1.0 data from the associated file" do
      table = HexaPDF::Font::TTF::Table::Maxp.new(@file, @entry)
    ...
    it "loads some default values if no entry is given" do
      table = HexaPDF::Font::TTF::Table::Maxp.new(@file)
      assert_equal(1, table.version)
