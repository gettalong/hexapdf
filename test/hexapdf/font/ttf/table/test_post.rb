...
require 'test_helper'
require 'stringio'
require 'hexapdf/font/ttf/table/post'

describe HexaPDF::Font::TTF::Table::Post do
  before do
    data = [1, 0, 1, 0, -142, 15, 0, 0, 0, 0, 0].pack('n4s>2N5')
    @file = Object.new
    @file.define_singleton_method(:io) { @io ||= StringIO.new(data) }
    @entry = HexaPDF::Font::TTF::Table::Directory::Entry.new('post', 0, 0, @file.io.length)
  end

  describe "initialize" do
    it "reads the format 1 data from the associated file" do
      table = HexaPDF::Font::TTF::Table::Post.new(@file, @entry)
    ...
    it "loads some default values if no entry is given" do
      table = HexaPDF::Font::TTF::Table::Post.new(@file)
      assert_equal(1, table.format)
    ...
    it "raises an error if an unsupported format is given" do
      ...
      assert_raises(HexaPDF::Error) { HexaPDF::Font::TTF::Table::Post.new(@file, @entry) }
    end
  end
end
