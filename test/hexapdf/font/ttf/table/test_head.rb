...
describe HexaPDF::Font::TTF::Table::Head do
  before do
    data = [1, 0, 2, 6554, 0, 42, 0x5f0f, 0x3CF5, 3, 64].pack('n*')
    @time = Time.new(2016, 05, 01)
    data << ([(@time - HexaPDF::Font::TTF::Table::TIME_EPOCH).to_i] * 2).pack('Q>*')
    data << [-132, -152, 3423, 4231, 3, 9, -2, 0, 0].pack('s>4n2s>3')
    io = StringIO.new(data)
    @file = Object.new
    @file.define_singleton_method(:io) { io }
    @entry = HexaPDF::Font::TTF::Table::Directory::Entry.new('head', 0, 0, io.length)
  end

  describe "initialize" do
    it "reads the data from the associated file" do
      table = HexaPDF::Font::TTF::Table::Head.new(@file, @entry)
    ...
    it "loads some default values if no entry is given" do
      table = HexaPDF::Font::TTF::Table::Head.new(@file)
    ...
    it "raises an error if the magic number is false when reading from a file" do
      ...
      assert_raises(HexaPDF::Error) { HexaPDF::Font::TTF::Table::Head.new(@file, @entry) }
    end
  ...
  describe "checksum_valid?" do
    ...
    it "raises an error if the checksum can't be verified because none is available" do
      assert_raises(HexaPDF::Error) { HexaPDF::Font::TTF::Table::Head.new(@file).checksum_valid? }
    end
  end
end
