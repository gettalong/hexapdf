...
require 'test_helper'
require 'hexapdf/font/afm'
require 'hexapdf/data_dir'
require 'tempfile'
require 'stringio'

describe HexaPDF::Font::AFM::Parser do
  describe "::parse" do
    before do
    ...
    it "can work with file names" do
      assert_equal('Test', HexaPDF::Font::AFM::Parser.parse(@file.path).font_name)
    end

    it "can work with IO streams" do
      @file.open
      assert_equal('Test', HexaPDF::Font::AFM::Parser.parse(@file).font_name)
    end
  end

  it "can parse the 14 core PDF font files" do
    Dir[File.join(HexaPDF.data_dir, 'afm', '*.afm')].each do |file|
      metrics = HexaPDF::Font::AFM::Parser.parse(file)
  ...
  it "fails if the file doesn't start with the correct line" do
    ...
    assert_raises(HexaPDF::Error) { HexaPDF::Font::AFM::Parser.parse(file) }
  end
end
